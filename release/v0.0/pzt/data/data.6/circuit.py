...
class CircuitModel ():
  ...
  def residualsZ (this, params, freq, mag, phase, phaseweight = 1e0):
    M, P = this.modelZ (params, freq)
    Merror = (M - mag) / mag
    Perror = (P - phase) / phase
    return np.concatenate ([Merror, Perror * phaseweight])

  def residualsY (this, params, freq, mag, phase, phaseweight = 1e0):
...
class IdealPiezoModel (CircuitModel):
  def __init__ (this, id, names):
    super ().__init__ (id, names)
    this.initial = [7e-9, 5e3, 20, 1e-9]
    this.bounds = ([500e-12, 1, 1e-6, 1e-12], [100e-6, 1e9, 1e6, 10e-6])
