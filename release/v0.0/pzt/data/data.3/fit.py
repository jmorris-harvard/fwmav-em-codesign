...
class App (tk.Tk):
  def __init__ (this, models, data, admittance):
    ...
    if this.admittance:
      ...
      this.measured_y_phase = data['phase (rad)'].to_numpy () * (180.0 / np.pi)
    else:
      this.measured_y_magnitude = data['impedance (Ohm)'].to_numpy ()
      this.measured_y_phase = data['phase (rad)'].to_numpy () * (180.0 / np.pi) * -1.0
    ...
    this.scatter_magnitude = this.ax_magnitude.scatter (this.measured_x, this.measured_y_magnitude, s = 100, edgecolors = 'black', label = 'measured', color = 'gray')
    this.curves_magnitude = list ()
    this.colors = ['#009e73', '#d55e00', '#cc97a7', '#0072b2']
    for i, model in enumerate (models):
      this.curves_magnitude.append (this.ax_magnitude.plot (list (), list (), linewidth = 3, color = this.colors[i], label = model.id)[0])
    ...
    for i, model in enumerate (models):
      this.curves_phase.append (this.ax_phase.plot (list (), list (), linewidth = 3, color = this.colors[i], label = model.id)[0])
    # this.ax_phase.grid ()
...
def parse_arguments ():
  ...
  parser.add_argument ('--models', nargs = '+', help = 'model type', default = ['idealpiezo:bimorph:c0,r1,l1,c1'])
  parser.add_argument ('--length', help = 'actuator length', default = 10.0)
  parser.add_argument ('--width', help = 'actuator width', default = 5.0)
