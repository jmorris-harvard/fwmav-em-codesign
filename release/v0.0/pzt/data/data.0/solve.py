...
class CircuitModel ():
  ...
  def residuals (this, params, freq, mag, phase, phaseweight = 300e3):
    M, P = this.model (params, freq)
    Merror = M - mag
    Perror = P - phase
...
class PiezoModel (CircuitModel):
  def __init__ (this):
    ...
    this.bounds = ([0.01, 1e-18, 0.01, 1e-18, 1e-12], [1e9, 1, 1e9, 1, 1e9])
...
def main ():
  ...
  model.initialize ((4.5e6, 2e-9, 25e3, 2e-9, 750e-3))
  ...
  inp = result.x
  inp = (4.5e6, 2e-9, 25e3, 2e-9, 750e-3)
  model.print (inp)
  Mmodel, Pmodel = model.model (inp, frequency_smooth)
