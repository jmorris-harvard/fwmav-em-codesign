...
module chargepump #(
	...
	// Input Registers
	input		[DATA_WIDTH - 1:0]		I_divider,

	// GPIO
	output		[1:0]		chargepump_drive
);
	// Routing
	// -- Triggers
	// ---- Bits
...
module chargepump_register_io #(
	...
	chargepump #(
		.DATA_WIDTH (DATA_WIDTH)	) chargepump_inst (
		.CLK (CLK),
		.RESETn (RESETn),
		...
		.chargepump_drive (chargepump_drive)
	);

endmodule

module chargepump_ahb_interpreter #(
	parameter integer DATA_WIDTH = 32,
...
module chargepump_ahb #(
	...
	// Instantiations
	chargepump_ahb_interpreter #(
		.DATA_WIDTH (DATA_WIDTH)
	) chargepump_ahb_interpreter_inst (
		.HCLK (HCLK),
		.HRESETn (HRESETn),
		.HADDR (HADDR),
		.HBURST (HBURST),
	...
	chargepump_register_io #(
		.DATA_WIDTH (DATA_WIDTH)
	) chargepump_register_io_inst (
		.CLK (HCLK),
		.RESETn (HRESETn),
