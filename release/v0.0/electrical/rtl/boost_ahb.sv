...
module boost #(
	...
	// Triggers
	input		[DATA_WIDTH - 1:0]		T_T0, // 0x0

	// Flags
	output		[DATA_WIDTH - 1:0]		F_F0, // 0x4

	// Input Registers
	input		[DATA_WIDTH - 1:0]		I_divider_boost, // 0x8
	input		[DATA_WIDTH - 1:0]		I_adc_threshold, // 0xC
	...
	output		[0:0]		sclk,
	input		[0:0]		cmp,
	output		[0:0]		drive_boost
);
	// Routing
	// -- Triggers
	// ---- Bits
	wire T_T0_b0_start_boost;
	assign T_T0_b0_start_boost = T_T0[0];
	wire T_T0_b1_send_adc;
	assign T_T0_b1_send_adc = T_T0[1];

	// -- Flags
	localparam integer F_F0_FLAG_BIT_N = 1;

	// ---- Bits
	wire F_F0_b0_running_boost;

	// ---- Locations
	assign F_F0[F_F0_FLAG_BIT_N - 1:0] = {F_F0_b0_running_boost};
  ...
  boostctl boostctl_inst (
    .clk (CLK),
    .rstn (RESETn),

    .boost_divider (I_divider_boost[15:0]),

    .boost_trigger (T_T0_b0_start_boost),
    .boost_running (F_F0_b0_running_boost),

    .adc_send (T_T0_b1_send_adc),
    .adc_threshold (I_adc_threshold[7:0]),

    .boost_drive (drive_boost),
    .cmp (cmp),
    .csn (csn),
    .sdio (sdio),
    .sclk (sclk)
...
module boost_register_io #(
	...
	boost #(
		.DATA_WIDTH (DATA_WIDTH)	) boost_inst (
		.CLK (CLK),
		.RESETn (RESETn),

		.T_T0 (triggers[0]),
		.F_F0 (flags[0]),
		.I_divider_boost (iregs[0]),
		.I_adc_threshold (iregs[1]),
...
module boost_ahb #(
	...
	// Instantiations
	boost_ahb_interpreter #(
		.DATA_WIDTH (DATA_WIDTH)
	) boost_ahb_interpreter_inst (
		.HCLK (HCLK),
		.HRESETn (HRESETn),
		.HADDR (HADDR),
		.HBURST (HBURST),
